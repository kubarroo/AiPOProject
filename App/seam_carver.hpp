...
class SeamCarver {
  public:
    ...
    void setImage(const cv::Mat &mat) {
        baseImage = mat.clone();
        lastCachedResult = {.width = mat.cols, .height = mat.rows, .result = mat.clone()};
    }
    ...
    cv::Mat getImage(int targetWidth, int targetHeight) {
        ...
        if (isCacheMissInAxis(baseImage.cols, targetWidth, lastCachedResult.width) ||
            isCacheMissInAxis(baseImage.rows, targetHeight, lastCachedResult.height)) {
            std::cerr << "[cache miss]";
            return changeResolutionAndCache(baseImage, targetWidth, targetHeight);
        }
        return changeResolutionAndCache(lastCachedResult.result, targetWidth, targetHeight);
    }
  ...
  private:
    cv::Mat baseImage;
    LastCachedResult lastCachedResult = {.width = 0, .height = 0, .result = cv::Mat()};
    ...
    cv::Mat changeResolutionAndCache(cv::Mat img, int targetWidth, int targetHeight) {
    ...
    void computeEnergy(const cv::Mat &img, cv::Mat &gray, cv::Mat &grad_x, cv::Mat &grad_y,
                       cv::Mat &energy) {
        int H = img.rows;
        int W = img.cols;
...
#pragma omp parallel for schedule(static)
    ...
    void findSingleSeam(const cv::Mat &energy, cv::Mat &M, cv::Mat &backtrack,
                        std::vector<int> &seam) {
        int H = energy.rows;
        int W = energy.cols;
...
#pragma omp parallel for schedule(static)
        for (int i = 0; i < H; ++i) {
            ...
            int col = seam[i];
            if (col < W - 1) {
                std::memmove(row_ptr + col * esz, row_ptr + (col + 1) * esz, (W - 1 - col) * esz);
            }
        }
